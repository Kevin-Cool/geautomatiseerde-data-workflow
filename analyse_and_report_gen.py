...
def combined(df_daily):
    ...
    fig.savefig(f'{analysed_data_dir}/combined.png') 
    plt.cla()
    plt.clf()
    ...
    left_image = f".{analysed_data_dir}combined.png"
...
def separate(df_daily_complex):
    ...
    plt.xticks(rotation=15)
    fig = plot.get_figure()
    fig.savefig(f'{analysed_data_dir}/separated.png')  
    plt.cla()
    plt.clf()
...
def simple_template(file,titel,intro,left_image,right_text):
    with  open(f"{report_data_dir}/{file}.md", "w") as md:
        md.write(f'# {titel}\n')
...
if __name__ == "__main__":
    # catch any given arguments 
    args = sys.argv[1:]
    if(len(args)>0):
        pars_arguments(args)
    # get clean data file
    df  = pd.read_csv(f'{clean_data_dir}/cleandata.csv')
